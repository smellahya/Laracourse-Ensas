...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Anouncements</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('anounecement.create') }}" class="bg-gray-0 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
                    <button type="button" class="btn btn-outline-success">
                        <span class="ml-2 text-xs font-semibold"> Add Anounecement</span>
                    </button>
                </a>
            </div>
        </div>
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-3/12 px-4 py-3">Details</div>
                <div class="w-2/12 px-4 py-3 text-right">Action</div>
            </div>
            @foreach ($anouncements as $anouncement)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $anouncement->title }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight"><img src='{{'asset(Desktop\pictures'. $anouncement->image }}' /></div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $anouncement->details}}</div>
                    
                    <div class="w-3/12 flex items-center justify-end px-3">
                        <a href="{{ route('subject.edit',$anouncement->id) }}">
                        ...
                        <form action="{{ route('anounecement.destroy',$anouncement->id) }}" method="POST" class="inline-flex ml-1">
                            @csrf
                            @method('DELETE')
